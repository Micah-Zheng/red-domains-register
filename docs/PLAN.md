...
| 档 | CAA | 理由 |
| :--- | :--- | :--- |
| A（橙云） | `0 issue ";"` + `0 issuewild ";"` | 阻止批量注册刷证书抽干共享预算；橙云用户不需自有证书，零成本 |
| B（灰云非 HTTP） | 同上 | 不需公信证书，**配额消耗结构性为零**，不靠承诺 |
| C（灰云自签 HTTPS） | **不下发**阻挡性 CAA | 否则用户无法签证书 |

CAA 由 CA 在签发时树形上溯取最近祖先（RFC 8659），故子名字上的 deny 只影响该名字，
不影响 CF 在 `tcp.red` 层签发 `*.tcp.red`。档位切换（B→C 或 C→B）须同步增删该记录。
...
| 记录 | 归属 | 漏掉的后果 |
| :--- | :--- | :--- |
| `<前缀>` CAA | A / B 档随主记录下发（§8.4） | **B 档配额保护静默失效**，且无任何报错 |
