...
| `description` | 5–200 字符；NFKC 归一化后剥离控制字符与零宽字符。**不校验用途与 zone 的关系**（D1） |
...
| `proxied` | 布尔。`true` 时 TTL 强制 1(auto)，SSL 模式须 Full (strict) |
| `check.mode` | 必填，`dns` / `http` / `tcp` / `manual`（D4）。A/AAAA **不得**为 `dns` |
| `check.port` | `http` / `tcp` 模式必填，1–65535 |
...
1. **两个 zone 完全等价**，只是名字偏好。用 `udp.red` 建网站、用 `tcp.red` 跑 WireGuard
   都完全合规，审核**不看**用途与协议名是否匹配。按哪个好念、好记来选即可。
...
已拍板的六项见 §1。以下三项需要真实数据或外部结果才能决定，留待对应阶段：
